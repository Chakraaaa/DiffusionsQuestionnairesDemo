...
<div>
    <div class="container">
        ...
        <main style="background-color: white">
            ...
            <div class="questions" style="margin-top: 150px">
                <?php
    $chapterNumber = 0;
    $questionNumber = 0;
    ...
    foreach ($questions as $index => $question) {
                ...
                <div class="question" id="question_<?= $question->id ?>"
                     style="margin-bottom: <?php if ($question->questionType == 'CHAPTER') { ?>20px<?php } else { ?>100px<?php } ?>;">
                    <?php
        switch (strtoupper($question->questionType)) {
                    case 'CHAPTER': ?>
                    <h2 style="font-size: 19px; color: #ff6600;font-family: Trebuchet MS; font-weight: bold; display: flex; align-items: center;">
                        <span style="margin-left: 5px"><?= $question->label ?></span>
                    </h2>
                    <?php break;

            case 'INPUT-TEXT': ?>
                    <div style="top: 24px; position: relative"><?= $questionNumber ?></div>
                    <input type="text"
            ...
            case 'INPUT-RADIO': ?>
                    <div>
                        <div style="top: 25px; position: relative"><?= $questionNumber ?>.</div>
                        <div style="margin-left: 25px"><?= $question->label ?></div>
                        <div class="has-text-right" style="margin-top: 15px">
                            <?php
                        $radioOptions = [
                            'PDTV' => 'Pas du tout vrai',
                            'PPV' => 'Plutôt pas vrai',
                            'PV' => 'Plutôt vrai',
                            'TAFV' => 'Tout à fait vrai'
                            ];
                            foreach ($radioOptions as $value => $label) {
                            ?>
                            <div class="radio-container">
                                <span class="radio-number"><?= $label ?></span>
                                <label class="radio">
                                    <input type="radio"
                                           name="question_<?= $question->id ?>"
                                           value="<?= $value ?>"
                                           class="radio-input"
                                            <?php if($modeTest==0) { ?>
                                                <?= ($question->quizUserResponse->value != '' && $question->quizUserResponse->value == $value) ? 'checked' : '' ?>
                                            <?php } ?>
                                    >
                        ...
                        data-question="question_<?= $question->id ?>" data-next-question="question_<?= $arrayQuestionAndNextQuestion[$question->id] ?>"
                        <?php } else { ?>
                        data-question="question_<?= $question->id ?>" data-next-question="critere_1"
                        <?php } ?>>
                        Ok
                        </button>
                    </div>
            ...
            case 'TEXT': ?>
                    [<?= $chapterNumber ?>.<?= $questionNumber ?>] - <?= $questionLabel ?>
                    <?php break;
        }
        ?>
                </div>
            ...
            <div class="criteres">
                <div style="font-size: 19px; color: #ff6600;font-family:Trebuchet MS;font-weight: bold;margin-bottom: 20px">CARACTÉRISTIQUES INDIVIDUELLES</div>
                <?php
                $criteresCount = count(array_filter($criteres, function($critere) {
                    return !empty($critere->titre);
                }));
                $currentIndex = 0;
                foreach ($criteres as $index => $critere) {
                if ($index === 'id' || empty($critere->titre)) continue;
                $currentIndex++;
                ?>
                <div class="critere" id="critere_<?= $index ?>" style="<?php if ($index === $criteresCount) { ?>margin-bottom: 100px;<?php } else { ?>margin-bottom: 50px;<?php } ?>">
                    <div>
                        <label class="label" style="display: inline-block; margin-bottom: 20px;"><?= htmlspecialchars($critere->titre) ?></label>
                    </div>
                ...
                $choices = [];
                for ($i = 1; $i <= 10; $i++) {
                    $choix = $critere->{"choix$i"};
                        if (!empty($choix)) {
                        $choices[] = $choix;
                        echo "<p><strong>" . $alphabet[count($choices) - 1] . "</strong> - " . htmlspecialchars($choix) . "</p>";
                        }
                        }
                        ?>
                    </div>
                    <div class="has-text-right">
                        <?php foreach ($choices as $key => $choix) {
                        $responseKey = "responseCritere" . ($index);
                        $isChecked = isset($responseQuizCriteresBarometre->$responseKey) && $responseQuizCriteresBarometre->$responseKey === $alphabet[$key];
                        ?>
                        <div class="radio-container">
                            <span class="radio-number"><?= $alphabet[$key] ?></span>
                            <label class="radio">
                                <input type="checkbox"
                                       name="critere_<?= $index ?>"
                                       value="<?= $alphabet[$key] ?>"
                    ...
                    <button class="button btn-ok mt-5 is-pulled-right"
                    <?php if ($currentIndex === $criteresCount) { ?>
                        data-question="critere_<?= $index ?>"
                    <?php } else { ?>
                        data-question="critere_<?= $index ?>" data-next-question="<?= ($currentIndex === $criteresCount) ? 'que' : 'critere_' . ($index + 1) ?>"
                    <?php } ?>>
                        Ok
                    </button>
                </div>
            ...
            <div class="has-text-right mt-5 mr-3">
...
<script>
    $(document).ready(function() {
        $('#submitQuizButton').on('click', function() {
            ...
            $('.criteres .critere').each(function () {
            ...
            if (allAnswered) {
                ...
                if(unansweredCount === 1) {
                    alert("Vous n'avez pas répondu à " + unansweredCount + " question ou critère.");
                } else {
                    alert("Vous n'avez pas répondu à " + unansweredCount + " questions ou critères.");
                }
        ...
        $('input[type="radio"]').on('change', function() {
            ...
            const container = radioInput.closest('.question');
            container.find('.radio-container').removeClass('error');
            const questionId = $(this).attr('name').split('_')[1];
            const response = $(this).val();
            const quizUserId = '<?=$quizUser->id ?>';
            var url = '<?=$urlSaveResponse?>';
        ...
        $('input[type="checkbox"]').on('change', function() {
            const radioInput = $(this);
            const container = radioInput.closest('.critere');
            container.find('.radio-container').removeClass('error');
            const numeroCritere = $(this).attr('name').split('_')[1];
            const response = $(this).val();
            const quizUserId = '<?=$quizUser->id ?>';
            const quizCriteresBarometreId = '<?=$criteres["id"]?>';
            var url = '<?=$urlSaveCritere?>';

            $('input[type="checkbox"][name="critere_' + numeroCritere + '"]').not(this).prop('checked', false);

            $.ajax({
                url: url,
                type: 'GET',
                data: {
                    'numeroCritere': numeroCritere,
                    'response': response,
                    'quizUserId': quizUserId,
                    'quizCriteresBarometreId': quizCriteresBarometreId
                },
                success: function(response) {
                    console.log(response);
                    if (response.status === 'success') {
