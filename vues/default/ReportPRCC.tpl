...
<div class="columns">
    ...
    <div class="column is-10">
        <div class="form-container">
            ...
            <?php foreach ($quizUsers as $quizUser) { ?>

                <div class="column is-two-thirds" style="margin-top: 25px">

                    <div>
                        <h2 style="margin-bottom: 10px;margin-left: 3px;" class="title is-6">Répondant : <?= $quizUser->userFirstName ?> <?= $quizUser->userLastName ?> (<?= $quizUser->userEmail ?>)</h2>
                    </div>
                    <div class="has-text-left">
                        <?php if($quizUser->status == 'FINISH') { ?>
                            <a id="btn-generate-report" class="button-valider" title="Générer le rapport" href="<?=WEB_PATH?>report.html/generate<?=$quiz->type?>?quizId=<?=$quiz->id?>&quizUserId=<?=$quizUser->id?>" onclick="buttonAppearance('#btn-generate-report','fas fa-file-word','Générer le rapport','1000')"><i class="fas fa-file-word"></i>&nbsp;Générer le rapport</a>
                        <?php } else { ?>
                            <a style="opacity: 0.5;cursor: default" class="button-valider" title="Générer le rapport"><i class="fas fa-file-word"></i>&nbsp;Générer le rapport</a>
                        <?php } ?>
                ...
                <div class="column is-two-thirds">
                    <div class="has-text-left">
                        <?php if($quizUser->status == 'FINISH') { ?>
                            <a id="btn-generate-report-garde" class="button-valider" title="Générer le rapport" href="<?=WEB_PATH?>report.html/generate<?=$quiz->type?>?quizId=<?=$quiz->id?>&quizUserId=<?=$quizUser->id?>&pagegarde=1" onclick="buttonAppearance('#btn-generate-report-garde','fas fa-file-word','Générer le rapport avec page de garde ','1000')"><i class="fas fa-file-word"></i>&nbsp;Générer le rapport avec page de garde</a>
                        <?php } else { ?>
                            <a style="opacity: 0.5;cursor: default" class="button-valider" title="Générer le rapport avec page de garde"><i class="fas fa-file-word"></i>&nbsp;Générer le rapport avec page de garde</a>
                        <?php } ?>
                    </div>
                </div>
                <div class="column is-two-thirds">
                    <div class="has-text-left">
                        <?php if($quizUser->status == 'FINISH') { ?>
                            <a id="btn-generate-data" class="btn-download-result-csv button-valider" title="Télécharger les résultats bruts" href="<?=WEB_PATH?>report.html/data<?=$quiz->type?>?quizId=<?=$quiz->id?>&quizUserId=<?=$quizUser->id?>" onclick="buttonAppearance('#btn-generate-data','far fa-file-excel','Télécharger les résultats bruts','1000')"><i class="far fa-file-excel"></i>&nbsp;Télécharger les résultats bruts</a>
                        <?php } else { ?>
                            <a style="opacity: 0.5;cursor: default" class="button-valider" title="Générer le rapport avec page de garde"><i class="fas fa-file-word"></i>&nbsp;Télécharger les résultats bruts</a>
                        <?php } ?>
